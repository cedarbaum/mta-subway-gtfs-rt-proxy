...
const createMatchTripUpdate = (cfg) => {
	...
	const _queryScheduleStopTimesForTripUpdate = async (tripUpdate) => {
		const {
			route_id,
			start_date,
			trip_id,
		...
		// todo: try to match the GTFS Schedule trip ID exactly using the following rules:
...
`,
			values: [
				...
				// Compared to GTFS Realtime trip IDs, the GTFS Schedule ones additionally have a prefix, for example:
				// - `072150_1..S03R` in GTFS Realtime
	...
	const matchTripUpdate = async (tripUpdate) => {
		const {
			route_id,
		...
		const scheduleStopTimes = await _queryScheduleStopTimesForTripUpdate(tripUpdate)
		const queryTime = Math.round(performance.now() - t0)
		// todo: add a metric for DB query time

		if (scheduleStopTimes.length === 0) {
			// todo: add a metric match/fail
			logger.warn({
				...logCtx,
				queryTime,
		...
		logger.debug({
			...logCtx,
			queryTime,
		}, 'found matching schedule trip for TripUpdate')
		...
		// todo: fill VehicleDescriptor.id using nyctTripDescriptor.train_id
	}
