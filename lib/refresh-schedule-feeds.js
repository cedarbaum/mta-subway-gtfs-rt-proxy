...
// We also make sure to always keep the DB currently marked as the latest one.
const determineDbsToRetain = (oldDbs, latestImport) => {
	// We mirror https://github.com/mobidata-bw/postgis-gtfs-importer/blob/76b148e/import.js#L62-L65 here.
	const mostRecent = sortBy(oldDbs, ['importedAt']).slice(-(MAX_SCHEDULE_DBS - 1))
	return latestImport && !mostRecent.find(db => db.name === latestImport.name)
		? [latestImport, ...mostRecent.slice(-1)]
		: mostRecent
}
