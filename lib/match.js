...
const createParseAndProcessFeed = async (cfg) => {
			...
			const matchFeedEntity = async () => {
				...
				try {
					if (feedEntity.trip_update) {
						await matchTripUpdateOrTransformToAdded(feedEntity.trip_update, {
							now,
							realtimeFeedName,
						})
					}
					if (feedEntity.vehicle) {
						await matchVehiclePositionOrTransformToAdded(feedEntity.vehicle, {
							realtimeFeedName,
						})
					}
					if (feedEntity.alert) {
	...
	return {
		...
		stop,
		checkIfHealthy,
		matchTripUpdate: matchTripUpdateOrTransformToAdded,
		matchVehiclePosition: matchVehiclePositionOrTransformToAdded,
		matchAlert,
		matchFeedMessage,
		storeStopTimeUpdatesInDb,
		restoreStopTimeUpdatesFromDb,
